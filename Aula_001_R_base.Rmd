...
Marcson Azevedo editou um pouco o material para estudar
...
$$10^2 + \frac{3*60}{8} - 3 = 119.5$$
```{r}
10^2+3*60/8-3
```

$$\frac{5^3*(6-2)}{61 - 3 +4} = 8.064516$$
```{r}
5^3*(6-2)/(61-3+4)
```

$$2^{2+1} - 4 + 64^{-2^{2.25}-1/4} = 16777220$$
```{r}
2^(2+1) - 4 + 64^((-2)^(2.25-1/4))
```

$$\left( \frac{0.44*(1-0.44)}{34}  \right)^{1/2} = 0.08512966 $$
```{r}
(0.44*(1-0.44)/34)^(1/2)
```
...
```{r}
foo = 32.1
my.vec2 = c(3,-3,2,3.45,1e+03,64^0.5,2+(3-1.1)/9.44,foo)
my.vec2
```

Ou juntar dois vetores:
```{r}
my.vec3 = c(myvec,myvec2)
...
```{r}
foo = 5.3
my.seq = seq(from=foo,to=(-47+1.5),by=-2.4)
my.seq
```

```{r}
my.seq2 = seq(from=foo,to=(-47+1.5),length.out = 5)
my.seq2
```
...
```{r}
my.vec[length(x=my.vec)]
```

```{r}
my.vec.len = length(x=my.vec)
bar = my.vec[my.vec.len-1]
bar
```
...
```{r}
my.vec[-1]  #ira excluir o primeiro elemento do vetor
```
...
```{r}
my.mat = rbind(c(1,3,4),5:3,c(100,20,90),11:13)
my.mat
```

```{r}
dim(my.mat) #retorna numero de linhas e colunas da matriz

nrow(my.mat) #retorna o numero de linhas da matriz

ncol(my.mat) #retorna o numero de colunas

dim(my.mat)[2]  #retorna o numero de colunas
```
...
```{r}
my.data = data.frame(Nome=c("Luis","Helena","Paulo","Catarina","Sara"),
...
```{r}
new.record = data.frame(
  person="Brian",
  age=7,
  sex=factor("M",levels=levels(my.data$sex)))
new.record
```

```{r}
my.data = rbind(my.data,new.record)
my.data
```
...
```{r}
my.data = cbind(my.data,funny)
my.data
```

```{r}
my.data$age.mon = my.data$age*12
my.data
```
...
```{r}
my.data[my.data$sex=="M",]
```

```{r}
my.data[my.data$age>15,]
```
